...
public class Reactor_40Router
{
    ...
    REcanvas Menu()
    {
        return new REcanvas
        {
            propsElementId = () => new ElementIdSetter { id = "Menu" },
    ...
    REcanvas Home()
    {
        return new REcanvas
        {
            propsElementId = () => new ElementIdSetter { id = "Home" },
    ...
    REcanvas Settings()
    {
        return new REcanvas
        {
            propsElementId = () => new ElementIdSetter { id = "Settings" },
    ...
    REcanvas Default()
    {
        return new REcanvas
        {
            propsElementId = () => new ElementIdSetter { id = "Default" },
    ...
    public IEnumerator ReactorRouter_10Basics_HideMode_NoPreDraw()
    {
        ...
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1);
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).canvas.enabled); // The canvas is enabled
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Only is one in scene
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).canvas.enabled); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).canvas.enabled); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Still existing, but disabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        yield return new WaitForSecondsRealtime(.1f); // Need  to wait a bit, because canvas is enabled at next frame, no immediatly
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1); // Still existing, but disabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Still existing, but disabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).canvas.enabled); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).canvas.enabled); // The canvas is disabled
        ...
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Default").root).canvas.enabled); // The canvas is disabled
        ...
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).canvas.enabled); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Default").root).canvas.enabled); // The canvas is disabled
    ...
    public IEnumerator ReactorRouter_10Basics_DisableMode_NoPreDraw()
    {
        ...
        yield return new WaitForSecondsRealtime(.1f); // Need  to wait a bit, because canvas is enabled at next frame, no immediatly
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1);
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).gameObject.activeSelf); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0);  // Only is one in scene
        ...
        yield return new WaitForSecondsRealtime(.1f); // Need  to wait a bit, because canvas is enabled at next frame, no immediatly
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1); // Still existing, but disabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Only is one in scene
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).gameObject.activeSelf); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Still existing, but disabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Still existing, but disabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 0); // Dont exist, because is not predrawed and are not routed yet
        ...
        yield return new WaitForSecondsRealtime(.1f); // Need  to wait a bit, because canvas is enabled at next frame, no immediatly
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1); // Still existing, but disabled
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Still existing, but disabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // The canvas is enabled
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is disabled
        ...
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Default").root).gameObject.activeSelf); // The canvas is disabled
        ...
        Assert.IsTrue(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is disabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Default").root).gameObject.activeSelf); // The canvas is disabled
    ...
    public IEnumerator ReactorRouter_10Basics_HideMode_WithPreDraw()
    {
        ...
        yield return new WaitForSecondsRealtime(.1f); // Need  to wait a bit, because canvas is enabled at next frame, no immediatly
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1);
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).canvas.enabled); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).canvas.enabled); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).canvas.enabled); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Default").root).canvas.enabled); // The canvas is enabled
    ...
    public IEnumerator ReactorRouter_10Basics_DisableMode_WithPreDraw()
    {
        ...
        yield return new WaitForSecondsRealtime(.1f); // Need  to wait a bit, because canvas is enabled at next frame, no immediatly
        Assert.IsTrue(REcanvas.Find("#Home").Length == 1);
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Home").root).gameObject.activeSelf); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Menu").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Menu").root).gameObject.activeSelf); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Settings").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Settings").root).gameObject.activeSelf); // The canvas is enabled
        Assert.IsTrue(REcanvas.Find("#Default").Length == 1); // Dont exist, because is not predrawed and are not routed yet
        Assert.IsFalse(((REcanvas.Selector)ReactorElement.FindOne("#Default").root).gameObject.activeSelf); // The canvas is enabled
