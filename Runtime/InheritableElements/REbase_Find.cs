    ...
    public abstract partial class REbase
    {
        ...
        public static REbaseSelector[] Find(string pattern)
        {
            ...
            if (String.IsNullOrEmpty(pattern))
            {
                ...
                return Resources.FindObjectsOfTypeAll<HC.ReactorId>().Select(s => s.selector).ToArray();
            }

            // By id
            else if (pattern.StartsWith("#") && pattern.TrimStart('#').Length > 0)
            {
                return Resources.FindObjectsOfTypeAll<HC.ReactorId>().Where(e => e.id == pattern.TrimStart('#')).Select(s => s.selector).ToArray();
            }
            ...
            else if (pattern.StartsWith(".") && pattern.Where(c => c == '|').Count() > 0 && pattern.Where(c => c == '&').Count() == 0)
            {
                ...
                return Resources.FindObjectsOfTypeAll<HC.ReactorId>().Where(elementIdCmp =>
                {
                    try
                    {
